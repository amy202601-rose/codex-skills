---
name: wfs-marketing-pb-cut-cn
description: Convert a Chinese Wallace Wang finance knowledge-slice oral script into publish-ready Chinese marketing fields, format the script for readability without changing any words, choose Chinese finance tags from the existing preferred tag pool, and write the issue into the WWFS Marketing publish center under "知识切片". Use when the user provides a 中文知识切片视频口播稿/script file or text and says "中文知识切片视频写入知识切片", "中文知识切片写入知识切片", "写入知识切片", "发布到知识切片", "新建中文知识切片一期", or asks to create Chinese title/cover/subtitle/tags/description for marketing.wallacewang.ca/publish.
---

# WFS Marketing PB Cut CN

Use this skill to turn a Chinese Wallace Wang finance knowledge-slice script into a new `知识切片` issue in the WWFS Marketing publish center.

## Inputs

- Chinese knowledge-slice oral script: usually a `.txt` file path or pasted Chinese口播稿.
- Title rules:
...
- Destination phrase: when the user says `中文知识切片视频写入知识切片`, `中文知识切片写入知识切片`, or `写入知识切片`, create or update an issue for the `知识切片` IP.
...
- Make the description a hook, not a summary. It should create curiosity and encourage watching the full clip or source video.
...
- Paragraph 3: explain why watching the whole clip/source video matters.
